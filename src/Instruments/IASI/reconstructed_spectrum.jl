...
#######  ReconstructedSpectrumDiskArray ##########

"""
    ReconstructedSpectrumDiskArray{T} <: AbstractMetopDiskArray{T, 4}

DiskArray to lazily compute the reconstructed IASI level 1 spectrum from Principal Component Scores (PCS).
"""
...
function ReconstructedSpectrumDiskArray(ds::MetopDataset{<:IASI_PCS_1C},
    ...
    dim_size = (
        size(m_reconstruct, 1), ds.dim["sounder_pixel"], ds.dim["xtrack"], ds.dim["atrack"])
...
"""
    reconstruct_iasi_spectrum(ds::MetopDataset{<:IASI_PCS_1C})

Create a `CMD.CFVariable` that contains the reconstructed IASI spectrum.
The `iasi_pcs_eigenvectors` artifact is need to reconstruct the spectrum from the Principal Component Scores.
This 200 MB artifact will be downloaded automatically and stored with other julia artifact.

# Example

```julia-repl
julia> test_file_IASI_PCS = joinpath(test_data_artifact, "IASI_PCS_1C_M01_20260319210859Z_cropped_10.nat");
julia> ds = MetopDataset(test_file_IASI_PCS);
gs1cspect_reconstruct (8461 × 4 × 30 × 10)
  Datatype:    Float32
  Dimensions:  spectral × sounder_pixel × xtrack × atrack
  Attributes:
   description          = Level 1C spectra reconstructed from principal components
   units                = W/m2/sr/m-1
   missing_value        = NaN
```
"""
function reconstruct_iasi_spectrum(ds::MetopDataset{<:IASI_PCS_1C})
    return reconstruct_iasi_spectrum(ds, _get_iasi_eigen_files(ds))
end

function reconstruct_iasi_spectrum(ds::MetopDataset{R},
        eigen_files::AbstractVector{<:AbstractString}) where {R <: IASI_PCS_1C_V10}
    d_type = ds.high_precision ? Float64 : Float32
    m_reconstruct = BlockDiagonal([d_type.(_get_iasi_pcs_reconstruction(f))
                                   for f in eigen_files])
    return reconstruct_iasi_spectrum(ds, m_reconstruct)
end

function reconstruct_iasi_spectrum(
        ds::MetopDataset{R}, m_reconstruct::BlockDiagonal) where {R <: IASI_PCS_1C_V10}
...
########  Helper functions to handle the eigenvectors (Principal component Vectors) ##########

function _get_iasi_eigen_files(ds::MetopDataset{IASI_PCS_1C_V10})
    sat = ds.attrib["spacecraft_id"]
    ev_filenames = [replace(
                        MetopDatasets.read_single_record(ds, MetopDatasets.GlobalExternalAuxiliary, i).content, sat => "Mxx")
                    for i in 1:3]

    eigen_artifact = LazyArtifacts.artifact"iasi_pcs_eigenvectors"

    return joinpath.(eigen_artifact, ev_filenames)
end

function _get_iasi_pcs_reconstruction(eigen_file)
    M_r = HDF5.h5open(eigen_file, "r") do ds_eigen
        return HDF5.read_dataset(ds_eigen, "ReconstructionOperator")
    end
    return M_r
end

############# Extend the CDM interface ############
# The interface should include computed variables gs1cspect_reconstruct and spectra_wavenumber
...
"""
    _reconstruct_iasi_pcs!(C_out::Array{T, 4}, 
        M_reconstruct::BlockDiagonal{T}, 
        raw_scores::NTuple{3,Tuple{Array{Int32, 4}, Array{Int16, 4}, Array{Int8, 4}}}, 
        score_scale_factor::NTuple{3,T}, 
        missing_values::Tuple{Int32, Int16, Int8},
        channel_range, 
        score_buffer = zeros(T,300))

Allocation free helper function to reconstruct the IASI L1 spectrum.
"""
    ...
    for l in axes(C_out, 4)
        for k in axes(C_out, 3)
            for j in axes(C_out, 2)
                ...
                _fill_obs_scores!(
                    score_buffer, raw_scores, score_scale_factor, missing_values, (j, k, l))
...
"""
    custom_block_view_mul!(C, M::BlockDiagonal, row_range::Union{UnitRange,StepRange}, B)

Custom function to avoid allocations in mul!(C,A::view(BlockDiagonal),B)
"""
...
"""
    _fill_obs_scores!(score_buffer, 
        raw_scores, 
        score_scale_factor, 
        missing_values, 
        obs_index)

Function to help unpack the IASI principal component scores. 
Handles fill values, concatenation and type conversion in a allocation free way.
"""
function _fill_obs_scores!(score_buffer::Vector{T},
    ...
    for b in eachindex(raw_scores, score_scale_factor)
        ...
        let
            ...
            @views raw_score_slice = score_band_part[
                :, obs_index[1], obs_index[2], obs_index[3]]
